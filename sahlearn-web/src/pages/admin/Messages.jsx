          ...
          <table className="w-full text-sm">
            <thead className="bg-surface-100 border-b border-ink-300/40">
              <tr>
                ...
                <th className="text-left px-4 py-3 font-medium text-ink-700 hidden md:table-cell">Subject</th>
            ...
            <tbody className="divide-y divide-ink-300/40">
              {messages.map((msg) => (
                <tr key={msg.id} className="hover:bg-surface-100/50 transition-colors">
                  <td className="px-4 py-3">
                    <p className="font-medium text-ink-900">{msg.name}</p>
                    <p className="text-ink-500 text-xs">{msg.email}</p>
                  </td>
                  <td className="px-4 py-3 text-ink-700 hidden md:table-cell line-clamp-1">{msg.subject}</td>
                  <td className="px-4 py-3 hidden sm:table-cell">
                    <span className={`px-2 py-0.5 rounded-full text-xs font-medium capitalize ${STATUS_BADGE[msg.status]}`}>
                      {msg.status}
                    </span>
                  </td>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center justify-end gap-2">
                      <button onClick={() => setSelected(msg)} className="p-1.5 text-ink-500 hover:text-brand-primary transition-colors">
                        <Eye size={15} />
                      </button>
                      <button onClick={() => setDeleteTarget({ id: msg.id, name: msg.name })} className="p-1.5 text-ink-500 hover:text-brand-danger transition-colors">
                        <Trash2 size={15} />
                      </button>
                    </div>
                  </td>
